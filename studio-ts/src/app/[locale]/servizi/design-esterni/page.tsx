...
import imageLaptop from '@/images/finitions-haut-de-gamme-paris.jpg'
import imageWhiteboard from '@/images/transformation-espace-design-paris.jpg'
import { RootLayout } from '@/components/RootLayout'
import { AvailableLocalesProvider } from '@/contexts/AvailableLocalesContext'
import { routes, locales } from '@/lib/routes'

export default function OutdoorDesignService() {
  ...
  return (
    ...
    <RootLayout>
          ...
          <div className="lg:flex lg:items-center lg:justify-end lg:gap-x-8 xl:gap-x-20">
            <div className="flex justify-center">
              <FadeIn className="w-135 flex-none lg:w-180">
                <StylizedImage
                  src={imageWhiteboard}
                  alt="Transformation d'espace par design à Paris"
          ...
          <div className="lg:flex lg:items-center lg:justify-start lg:gap-x-8 lg:group-even/section:justify-start xl:gap-x-20">
            <div className="flex justify-center">
              <FadeIn className="w-135 flex-none lg:w-180">
                <StylizedImage
                  src={imageLaptop}
                  alt="Finitions haut de gamme à Paris"
                  shape={1}
                  sizes="(min-width: 1024px) 41rem, 31rem"
      ...
      <div className="relative mt-24 pt-24 sm:mt-32 sm:pt-32 lg:mt-40 lg:pt-40">
        ...
        <Container className="mt-24">
          <GridList lgColumns={2}>
            <GridListItem title={t('gridList.terraces.title')}>
              {t('gridList.terraces.description')}
            </GridListItem>
            <GridListItem title={t('gridList.gardens.title')}>
              {t('gridList.gardens.description')}
            </GridListItem>
            <GridListItem title={t('gridList.furniture.title')}>
              {t('gridList.furniture.description')}
            </GridListItem>
            <GridListItem title={t('gridList.lighting.title')}>
              {t('gridList.lighting.description')}
            </GridListItem>
          </GridList>
        </Container>
      </div>
      ...
      <Container className="mt-24 sm:mt-32 lg:mt-40">
        <StatList>
          <StatListItem value="250+" label={t('stats.spaces')} />
          <StatListItem value="100%" label={t('stats.custom')} />
          <StatListItem value="4 stagioni" label={t('stats.seasons')} />
        </StatList>
      </Container>
